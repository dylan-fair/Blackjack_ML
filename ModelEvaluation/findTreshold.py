...
def model_decision(model, dealer_card, player_value, count, num_hits):
    ...
    # print(f"{prediction} is predicted value")
...
# action = ['stay', 'hit']
# print(action[model_decision(new_model, 10, 4, 0, 0)])
# print(action[model_decision(new_model, 5, 20, 0, 1)])
# print(action[model_decision(new_model, 9, 15, 0, 1)])
# print(action[model_decision(new_model, 11, 13, 0, 0)])

def find_threshold_ends():
    min = 0
    for i in range(2, 11):
        for j in range(17, 21):
            val = model_decision(new_model, i, j, 0, 0)
            if val > min:
                min = val
    ...
    for i in range(2, 11):
        for j in range(4, 11):
            ...
            if val < min:
                min = val
    print(f'the max value for threshold should be {min}')
    print(f"to ensure we hit when we want need threshold around max {max}, min {min}")
    print("")

    for i in range(0,5):
        print(f' :With a num hit of {i} the percentage is {model_decision(new_model, 10, 10, 0, i)}: ')
    print(" ")

    for i in range(-10,10):
        print(f' :With a count of {i} the percentage is {model_decision(new_model, 10, 10, i, 0)}: ')
